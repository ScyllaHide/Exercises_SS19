...
\begin{proof}
	\begin{itemize}
		...
		\begin{align*}
			...
			&(\subseteq)\quad \alpha = \frac{1}{a} (a\alpha + b) - \frac{b}{a}\\
		\end{align*}
		\item $K(\alpha) = K(\alpha^{-1}):$
		\begin{align*}
			&(\subseteq)\quad \alpha^{-1} = \frac{1}{a}\\
			&(\supseteq)\quad \alpha = (\alpha^{-1})^{-1}
		\end{align*}
...
\begin{proof}
	...
	\begin{enumerate}
		\item 
		\begin{enumerate}
			\item Sei $n \ge 1$. Es ist klar, dass $\sqrt[n]{2}$ eine Nullstelle von $X^n-2 \in \Q[X]$ ist. Diese Polynom ist normiert und irreduzibel über $\Q$ (EISENSTEIN, $p=2$). Deshalb ist $X^n -2$ das Minimalpolynom von $\sqrt[n]{2}$ über $\Q$.
...
Sei $\alpha \in L$ nicht algebraisch (= transzendent) über $K$ und $n \ge 1$. Zeigen Sie, dass $[K(\alpha):K(\alpha^n)] = n$.
